...
//	AFX_MANAGE_STATE(AfxGetStaticModuleState());
  ...
  // these command must be in left-to-right order of the toolbar buttons
	COMMANDDEFS s_cd[] = {
		{ "WTL ClassWizard", IDS_CMD_STRING, "WTLWizard" }, 
		{ "WINX ClassWizard", IDS_CMD_STRING, "WINXWizard" }, 
		{ "VisualFC Options ", IDS_CMD_STRING, "Options" }, 
	};
	...
	for (int n = 0; n < nNumCmds; n++)
	{
		...
		//  command are stored in the string table (IDS_CMD_STRING) and should
		//  be localized.
		CString szCommand = s_cd[n].lpszCommand;
		VARIANT_BOOL bRet;
		CString strCmdString;// = szCommand;
		//strCmdString.LoadString(s_cd[n].nStringID);
		strCmdString = szCommand +_T("\n")+szCommand+_T("\n")+szCommand+_T("\n")+szCommand;//+ strCmdString;
