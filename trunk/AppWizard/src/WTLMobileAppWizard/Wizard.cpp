...
//	wizard.AddSymbol(info.ar_app[info.nAppType],true);
	...
	wizard.AddSymbol(_T("WTL_FULLSCREEN"),info.m_bFullScreen);
	wizard.AddSymbol(_T("WTL_APP_DLG_ORIENT_AWARE"),info.m_bOrientAware);

	if (info.nAppType == 0)
		wizard.AddSymbol(_T("WTL_APPTYPE_SDI"),true);
	else if (info.nAppType == 1)
		wizard.AddSymbol(_T("WTL_APPTYPE_DLG"),true);

	if (info.m_nOrientType == 0)
		wizard.AddSymbol(_T("WTL_APP_DLG_RESIZE"),true);
	else if (info.m_nOrientType == 1)
		wizard.AddSymbol(_T("WTL_APP_DLG_ORIENT"),true);

	wizard.AddSymbol("WTL_USE_STRING",info.m_bUseString);

	if (info.m_nStringType == 0)
		wizard.AddSymbol("WTL_STRING_ATL",true);
	else if (info.m_nStringType == 1)
		wizard.AddSymbol("WTL_STRING_WTL",true);
	...
	wizard.AddSymbol("WTL_EVC_COMPAT",info.m_bEVCCompa);
	
	if (info.m_nMenuType == 0)
		wizard.AddSymbol("WTL_MENU_TYPE_2003",true);
	else if (info.m_nMenuType == 1)
		wizard.AddSymbol("WTL_MENU_TYPE_2005",true);
	else if (info.m_nMenuType == 2)
		wizard.AddSymbol("WTL_MENU_TYPE_BOTH",true);
	...
	CString strBaseName = wizard.FindSymbolVal("NICE_SAFE_PROJECT_NAME");

	if(wizard.FindSymbol("WTL_APPTYPE_SDI"))
	{
		CString strFrameFile = strBaseName + "Frame";
		CString strFrameClass = "C" + strFrameFile;
...
//			SetViewSymbols(strBaseName);
	...
	{
		CString strDlgFile = strBaseName + "Dialog";
		...
		CString BaseClassName = "CAppStdDialogImpl";
...
*/
	...
	{	
		...
		CString strViewClass;

		
		if(wizard.FindSymbol("WTL_USE_VIEW"))
		{
			CString strViewFile = strBaseName + "View";
		...
		CString strView = wizard.FindSymbolVal("WTL_COMBO_VIEW_TYPE");
		if (strView == "WTL_VIEWTYPE_FORM")
		{
		...
		else if (strView ==  "WTL_VIEWTYPE_PROPSHEET")
		{
			wizard.AddSymbol("WTL_USE_VIEW_CLASS", true);
			wizard.AddSymbol("WTL_VIEWTYPE_PROPSHEET", true);
		...
		else if (strView ==  "WTL_VIEWTYPE_AX")
		{
		...
		else if (strView == "WTL_VIEWTYPE_EDIT")
		{
		...
		else if(strView == "WTL_VIEWTYPE_LISTVIEW")
		{
		...
		else if(strView == "WTL_VIEWTYPE_TREEVIEW")
		{
		...
		else if (strView == "WTL_VIEWTYPE_HTML")
		{
		...
		else if (strView == "WTL_VIEWTYPE_INKX")
		{
		...
		else if (strView == "WTL_VIEWTYPE_RICHINK")
		{
			wizard.AddSymbol("WTL_VIEW_STYLES", "WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN  | IS_BOTTOMVOICEBAR");
			if(wizard.FindSymbol("WTL_USE_VIEW_CLASS"))
			{
...
CString CWizard::GetTargetName(const CString & strName, const CString & strProjectName)
{
	...
	{
		strTarget = wizard.FindSymbolVal("WTL_APPWND_FILE") +".h";
	}
	else if(strName == "frame.cpp" || strName == "MainDlg.cpp")
	{
		strTarget = wizard.FindSymbolVal("WTL_APPWND_FILE") +".cpp";
	}
	else if(strName == "view.h") 
	{
		strTarget = wizard.FindSymbolVal("WTL_VIEW_FILE") + ".h";
	}
	else if(strName == "view.cpp")
	{
		strTarget = wizard.FindSymbolVal("WTL_VIEW_FILE") + ".cpp";
	}
