...
public:
	...
	void DoUpdateUI(UINT id)
	{
		...
		UIEnable(IDC_REBAR,info->nAppType != 5 || info->bFrameDialog);
		UIEnable(IDC_CMDBAR,(info->nAppType != 5 || info->bFrameDialog) && info->bReBar);
		info->bCmdBar = info->bReBar;
		UIEnable(IDC_TOOLBAR,info->nAppType != 5 || info->bFrameDialog);
		UIEnable(IDC_STATUSBAR,info->nAppType != 5 || info->bFrameDialog);
