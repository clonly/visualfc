#ifndef _PAGE_WTL_DDX_H_
#define _PAGE_WTL_DDX_H_

struct page_wtl_ddx_peer : public page_ddx_peer
{
	virtual void init_tag(tag_token & tag)
	{
		...
		tag.add_body(_T("DDX_TEXT"));
		tag.add_body(_T("DDX_TEXT_LEN"));
		tag.add_body(_T("DDX_INT"));
		tag.add_body(_T("DDX_INT_RANGE"));
		tag.add_body(_T("DDX_UINT"));
		tag.add_body(_T("DDX_UINT_RANGE"));
		tag.add_body(_T("DDX_FLOAT"));
		tag.add_body(_T("DDX_FLOAT_RANGE"));
		tag.add_body(_T("DDX_FLOAT_P"));
		tag.add_body(_T("DDX_FLOAT_P_RANGE"));
		tag.add_body(_T("DDX_CHECK"));	
		tag.add_body(_T("DDX_RADIO"));	
		tag.add_head(_T("BEGIN_DDX_MAP"));
		tag.tag.begin = _T("BEGIN_DDX_MAP($CLS)");
		tag.tag.end = _T("END_DDX_MAP();");
	}
	virtual Token * add_token_dlg(codeparser * pcode, Token * tkcls, ResControl * ctrl)
	{
		...
		dlg.m_res = ctrl;
		dlg.get_var_type.bind(this,&page_wtl_ddx_peer::get_var_type);
		dlg.get_ddx_type.bind(this,&page_wtl_ddx_peer::get_ddx_type);
...
protected:
	void get_ddx_type(win32_notify * wn, ResControl * res, std::vector<CString> & array)
	{
		...
		{
			array.push_back(_T("DDX_TEXT"));
			array.push_back(_T("DDX_TEXT_LEN"));
			array.push_back(_T("DDX_INT"));
			array.push_back(_T("DDX_INT_RANGE"));
			array.push_back(_T("DDX_UINT"));
			array.push_back(_T("DDX_UINT_RANGE"));
			array.push_back(_T("DDX_FLOAT"));
			array.push_back(_T("DDX_FLOAT_RANGE"));
			array.push_back(_T("DDX_FLOAT_P"));
			array.push_back(_T("DDX_FLOAT_P_RANGE"));
		}
...
class page_wtl_ddx : public page_ddx_template
{
public:
	page_wtl_ddx()
	{
