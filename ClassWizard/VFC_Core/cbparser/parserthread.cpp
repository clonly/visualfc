...
wxChar ParserThread::SkipToOneOfChars(const wxString& chars, bool supportNesting, bool bGetValue)
{
	unsigned int level = m_Tokens.GetNestingLevel();
	while (1)
	{
		wxString token = m_Tokens.GetTokenExt(bGetValue);
		if (token.IsEmpty())
			return '\0'; // eof
...
//    Log("Added token " +name+ ", type '" +newToken->m_Type+ "', actual '" +newToken->m_ActualType+ "'");
	...
	if (kind == tkEnumerator)
	{
		newToken->m_DisplayName << newToken->m_Name << _T(" = ") << args;
	}
	else
...
void ParserThread::HandleClass(bool isClass)
{
	...
	if (theToken != NULL)
	{
		theToken->m_EndLine = m_Tokens.GetLineNumber();
	}
}
...
//        Log("Adding function '"+name+"': m_Str='"+m_Str+"'"+", enc_ns="+(m_EncounteredNamespaces.GetCount()?m_EncounteredNamespaces[0]:"nil"));
		//FIX 2007.4.12
		theToken = DoAddToken(kind, name, args, isOperator);
		//DoAddToken(kind, name, args, isOperator);
	}

	//FIX 2007.4.12
	if (theToken && 
		theToken->m_TokenKind == tkFunction &&
		theToken->m_Type.IsEmpty())
	{
		theToken->m_TokenKind = tkMapping;
	}
	else if (!m_Tokens.PeekToken().Matches(_T("}")))
		SkipToOneOfChars(_T(";}"),true);
	...
	while (1)
	{
		...
		if (peek.Matches(_T(",")) || peek.Matches(_T("}")) || peek.Matches(_T(":")))
		{
            ...
            if (isalpha(token.GetChar(0)))
            {
				...
				wxString value = m_Tokens.GetEnumToken();
                DoAddToken(tkEnumerator, token, value);
                m_pLastParent = lastParent;
			}
			if (peek.Matches(_T(":")))
			{
