...
class Tokenizer
{
	public:
		...
		wxString GetTokenExt(bool bGetValue);
		wxString GetToken();
		wxString GetEnumToken();
		wxString PeekToken();
		void UngetToken();
	...
	protected:
		void BaseInit();
		wxString DoGetToken(bool bGetValue = false);
		bool ReadFile();
		bool SkipWhiteSpace();
		...
		bool SkipUnwanted(bool bGetValue = false); // skips comments, assignments, preprocessor etc.
