...
bool CWizard::make_project()
{
	...
	wizard.AddSymbol("WTL_USE_CMDBAR",info.bCmdBar);
	...
	wizard.AddSymbol("WTL_USE_MANIFEST",info.bManifest);
	wizard.AddSymbol("WTL_HOST_AX",info.bHostActiveX);
	wizard.AddSymbol("WTL_USE_UNICODE",info.bUnicode);
	wizard.AddSymbol("WTL_USE_VIEW",info.bUseView);
	wizard.AddSymbol("PROJECT_LANG_"+info.ar_lang[info.nAppLang],true);

	wizard.AddSymbol("WTL_APPTYPE_DLG_FRAME",info.bFrameDialog);
	wizard.AddSymbol("WTL_USE_ADDVFCFILES",info.bAddVFCFiles);
	wizard.AddSymbol("WTL_USE_REBAR",info.bReBar);
